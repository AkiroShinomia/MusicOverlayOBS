...
public class FftProcessor
{
    ...
    public void SetOutputGain(double gain)
        {
            outputGain = Math.Clamp(gain, 0.1, 2.0);
        }
    ...
    private void CalculateFftBands()
    {
        ...
        for (int band = 0; band < BandCount; band++)
        {
            ...
            double presenceBoost =
                1.0 +
                Math.Sin(bandNorm * Math.PI) * 0.35;
...
double lowMidShape =
    1.0 +
    Math.Exp(-Math.Pow((bandPosition - 0.18) / 0.18, 2)) * 0.55;

double midShape =
    1.0 +
    Math.Exp(-Math.Pow((bandPosition - 0.52) / 0.22, 2)) * 0.35;

double highDamping =
    1.0 - Math.Pow(bandPosition, 1.7) * 0.28;
...
// Не даём per-band normalization превратить спектр в ровную линию
double normalized =
    globalNormalized * 0.72 +
    adaptiveNormalized * 0.28;

normalized = Math.Clamp(normalized * outputGain, 0, 1);

// Контраст
normalized = Math.Pow(normalized, spectralContrast);
normalized = Math.Pow(normalized, visualCurvePower);

// Усиление верхних нот / вокала / скрипки
double highPresence =
    1.0 + Math.Pow(bandPosition, 1.35) * 0.85;
